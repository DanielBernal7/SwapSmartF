...
function ComparisonTable({ scanned, detail, serving }: { scanned: ScannedProduct; detail: FoodDetail; serving: Serving | null }) {
	const altCal = serving?.calories ?? detail.calories;
	const altFat = serving?.total_fat ?? detail.total_fat;
	const altCarbs = serving?.total_carbs ?? detail.total_carbs;
	const altProtein = serving?.protein ?? detail.protein;
	const altSugar = serving?.total_sugars ?? detail.total_sugars;
	const altSodium = serving?.sodium ?? detail.sodium;
	const altSatFat = serving?.saturated_fat ?? detail.saturated_fat;
	const altTransFat = serving?.trans_fat ?? detail.trans_fat;
	const altPolyFat = serving?.polyunsaturated_fat ?? detail.polyunsaturated_fat;
	const altMonoFat = serving?.monounsaturated_fat ?? detail.monounsaturated_fat;
	const altCholesterol = serving?.cholesterol ?? detail.cholesterol;
	const altFiber = serving?.dietary_fiber ?? detail.dietary_fiber;
		...
		<View style={tableStyles.table}>
			<View style={tableStyles.tableHeader}>
				...
				<View style={tableStyles.valueCol}>
					<Text style={tableStyles.colHeader} numberOfLines={1}>
						Scanned
					</Text>
				</View>
				<View style={tableStyles.valueCol}>
					<Text style={[tableStyles.colHeader, { color: "#007AFF" }]} numberOfLines={1}>
						Alternative
					</Text>
				</View>
			</View>
			{rows.map((row, i) => (
				<View key={i} style={[tableStyles.row, i % 2 === 1 && tableStyles.rowAlt]}>
					<View style={tableStyles.labelCol}>
						<Text style={[tableStyles.rowLabel, row.isSubRow && tableStyles.subRowLabel, row.isSugar && tableStyles.sugarLabel]}>{row.label}</Text>
					</View>
					<View style={tableStyles.valueCol}>
						<Text style={[tableStyles.rowValue, row.isSugar && tableStyles.sugarValue]}>{row.scannedValue}</Text>
					</View>
					<View style={tableStyles.valueCol}>
						<Text style={[tableStyles.rowValue, { color: "#007AFF" }, row.isSugar && tableStyles.sugarValue]}>{row.altValue}</Text>
					</View>
				</View>
			))}
		</View>
	...
	return (
		<View style={styles.container}>
			...
			<View style={[styles.headerWrapper, { height: headerBarHeight }]} pointerEvents="box-none">
				...
				<View style={[styles.headerContent, { height: headerBarHeight, paddingTop: insets.top }]} pointerEvents="box-none">
					<Pressable onPress={() => router.back()} style={styles.backBtn}>
						<SymbolView name="chevron.left" style={styles.backIcon} tintColor="#007AFF" resizeMode="scaleAspectFit" weight="semibold" />
					</Pressable>
			...
			{!loading && !error && detail && (
				<ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: headerBarHeight + 20 }]} showsVerticalScrollIndicator={false}>
					...
					{detail.all_servings?.length > 1 && (
						<GlassCard style={styles.card}>
							<View style={styles.cardInner}>
								<ServingPicker servings={detail.all_servings} selectedIndex={selectedServingIndex} onSelect={setSelectedServingIndex} />
							</View>
						</GlassCard>
					)}
					...
					{scanned && (
						<GlassCard style={styles.card}>
							<View style={styles.cardInner}>
								<Text style={sectionStyles.sectionTitle}>Nutrition Comparison</Text>
								<ComparisonTable scanned={scanned} detail={detail} serving={currentServing} />
							</View>
						</GlassCard>
					)}
						...
						<GlassCard style={styles.card}>
							<View style={styles.cardInner}>
								...
								<View style={ingredientStyles.legend}>
									...
									<Text style={ingredientStyles.legendText}>Sugar aliases highlighted in red</Text>
								</View>
							</View>
						</GlassCard>
					)}
